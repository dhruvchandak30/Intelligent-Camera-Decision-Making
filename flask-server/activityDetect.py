...
for counter, random_index in enumerate(range(len(all_classes_names)), 1):
    ...
    video_files_names_list = os.listdir(
        f'/content/drive/MyDrive/dataset/train/{selected_class_Name}')
    ...
    video_reader = cv2.VideoCapture(
        f'/content/drive/MyDrive/dataset/train/{selected_class_Name}/{selected_video_file_name}')
...
early_stopping_callback = EarlyStopping(
    monitor='accuracy', patience=10, mode='max', restore_best_weights=True)
...
model_training_history = model.fit(x=features_train, y=labels_train, epochs=70, batch_size=4,
...
plot_metric(model_training_history, 'loss', 'val_loss',
            'Total Loss vs Total Validation Loss')

plot_metric(model_training_history, 'accuracy', 'val_accuracy',
...
for i in range(len(features_test)):
    predicted_label = np.argmax(model.predict(
        np.expand_dims(features_test[i], axis=0))[0])
...
predict_single_action(
    "/content/People Talking at The Table _ Free Video Loop.mp4", SEQUENCE_LENGTH)
